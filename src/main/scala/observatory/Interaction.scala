...
import observatory.Visualization.{createColorCalculator, predictTemperature}
...
object Interaction extends InteractionInterface {
  ...
  def tile(temperatures: Iterable[(Location, Temperature)], colors: Iterable[(Temperature, Color)], tile: Tile): Image = {
    val tile2Location = (tile: Tile) => tile.toLocation
    ...
    val pixels = tile
      .generateSubTiles(DEFAULT_ZOOM_LEVEL)
      .par
      .map(mapper)
      .toArray
